...
<div class="card shadow-lg p-4 rounded-3">
    ...
    <div class="mb-4">
        ...
        <table class="table table-borderless">
            ...
            <tr>
                <th>Subtotal</th>
                <td>Rp {{ number_format($penjualan->subtotal_nilai ?? 0, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <th>Margin</th>
                <td>{{ $penjualan->margin_persen ?? 0 }}%</td>
            </tr>
            <tr>
                <th>PPN</th>
                <td>{{ $penjualan->ppn ?? 10 }}%</td>
            </tr>
            <tr>
                <th>Total</th>
                <td>Rp {{ number_format($penjualan->total_nilai ?? 0, 0, ',', '.') }}</td>
            </tr>
        </table>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    selectBarang?.addEventListener('change', function() {
        ...
        if(stok !== '') {
            // optional: disable tambah bila stok 0
            if (parseInt(stok) <= 0) {
